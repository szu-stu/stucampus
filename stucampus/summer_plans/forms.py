...
class PlanForm(forms.ModelForm):
    content = forms.CharField(label=u"计划",required=True,error_messages={'required':u'不能为空'},
                            widget=forms.Textarea(attrs={'class':'form-control','placeholder':u'写下你的计划吧'}))
    email = forms.EmailField(label=u"邮箱",required=True,error_messages={'required':u'不能为空'},
