...
class FileField(forms.FileField):
    default_error_messages = {
        ...
        'max_length': ungettext_lazy(
            'chinese Ensure this filename has at most '
            '%(max)d character (it has %(length)d).',
            'chineseEnsure this filename has at most '
            '%(max)d characters (it has %(length)d.',
            'max'
        ),

        'contradiction': _('chinesePlease either submit a file'
...
class ImageField(forms.ImageField):
    default_error_messages = {
        'required': _(this_field_is_required),
        'invalid_image': _('chineseUpload a valid image. The file'
                           'you uploaded was either not an image or'
                           'a corrupted image.'),
        }
...
class ChoiceField(forms.ChoiceField):
    default_error_messages = {
        'required': _(this_field_is_required),
        'invalid_choice': _('chineseSelect a valid choice, %(value)s '
